...
class face_recognition:
  ...
  def __init__(self, model_path, thres=None, min_aligner_confidence=None):
    ...
    self.aligner = Aligner(min_aligner_confidence
                          ) if min_aligner_confidence is not None else Aligner(
                              config.min_aligner_confidence)
    self.feature_extractor = tf.keras.models.load_model(model_path +
                                                        "/model.h5",
                                                        compile=False)
  ...
  def calculate_distance(self, crop_img, db_faces_features, mode='avg'):
    ...
    crop_img_features = self.feature_extractor.predict(crop_img[None, :, :, :],
                                                       verbose=0)
    all_distances = [
    ...
    for face_idx in range(len(self.faces)):
      if mode == 'avg':
        db_face_features = db_faces_features[face_idx].mean(
            axis=0, keepdims=True)  # avg method
        new_crop_img_features = crop_img_features.copy()
      else:
        db_face_features = db_faces_features[face_idx]  # best method
        new_crop_img_features = np.tile(crop_img_features,
                                        [db_face_features.shape[0], 1])
      try:
      ...
      distance = np.max(self.new_distance(
          [db_face_features, new_crop_img_features]),
                        axis=0)

      if distance > self.thres:
        all_distances.append(
            distance)  # obj distance wrt to all faces in database
      else:
        all_distances.append(
            self.model_config.large_distance)  # not the person guaranteed
  ...
  def no_repeat_allowed_face_recognition(self, distance_dict):
    ...
    def assign_face_label(obj):
      ...
      if min_distance < self.thres:
        ...
        faceidx_to_obj_dict[min_distance_idx] = (obj, min_distance
                                                )  # stores obj and distance
      else:

        if (min_distance < faceidx_to_obj_dict[min_distance_idx][1]
           ):  # current is less matching
          distance_dict[obj][
              min_distance_idx] = self.model_config.large_distance
          assign_face_label(obj)
        else:  # current is more matching
          temp_obj, temp_min_distance = faceidx_to_obj_dict[min_distance_idx]
          faceidx_to_obj_dict[min_distance_idx] = (obj, min_distance
                                                  )  # stores obj and distance
          distance_dict[temp_obj][
              min_distance_idx] = self.model_config.large_distance
  ...
  def forward_pass(self, img, tree, mode="repeat"):
    ...
    for i, obj in enumerate(root.findall("object")):
      ...
      if crop_img is not None:
        self.distance_dict[obj] = np.array(
            self.calculate_distance(crop_img,
                                    self.db_faces_features,
                                    mode=self.distance_mode))

    # print(distance_dict)

    if mode == "repeat":
      faceidx_to_obj_dict = self.repeat_allowed_face_recognition(
          self.distance_dict)
    elif mode == "no-repeat":
      faceidx_to_obj_dict = self.no_repeat_allowed_face_recognition(
          self.distance_dict)
    return tree

  def predict(self, img, tree):
    if (not hasattr(self, "distance_mode")) or (not hasattr(
...
if __name__ == "__main__":
  ...
  for xml_file in glob(f"{img_dir}/*.xml"):
    ...
    cv2.imwrite(save_dir + "/" + root.find("filename").text,
