...
def add_row_user_table(username,
                       person_id,
                       face_vectors,
                       remarks,
                       group_id=None):
  ...
  if cursor.fetchone() is None:
    if group_id is not None:
      ...
      cursor.execute(
          query,
          [person_id, face_vectors.tobytes(), remarks, group_id])
    else:
    ...
    if group_id is not None:
      ...
      cursor.execute(query,
                     [face_vectors.tobytes(), remarks, group_id, person_id])
    else:
...
def read_user_table(username, split_remarks=True, group_id=None):
  ...
  if group_id is None:
    cursor.execute(
        f"select person_id,face_vectors,remarks,group_id from user_{username};")
  else:
    cursor.execute(
        f"select person_id,face_vectors,remarks,group_id from user_{username} where group_id=%s;",
        [group_id])
