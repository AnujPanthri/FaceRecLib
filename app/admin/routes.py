...
def get_random_unique_id():
  ...
  while (True):
    ...
    cursor.execute("select username from admins where session_token=%s",
                   [random_id])
...
def authenticate():
  dataBase = access_database_as_admin()
  cursor = dataBase.cursor()
  cursor.execute("select password from admins where username=%s",
  ...
  if None == db_password_hash:
    ...
    return redirect(
        url_for('admin.login_page_message', message="username doesn't exists"))
    ...
    dataBase = access_database_as_admin()
    cursor = dataBase.cursor()
    cursor.execute("update admins set session_token=%s where username=%s",
                   (session['admin_token'], request.form['username']))
    dataBase.commit()
    dataBase.close()
    ...
    # incorrect password
    return redirect(
        url_for('admin.login_page_message', message="Incorrect password"))
...
def is_auth(func):
  ...
  def wrapper_func(*args, **kwargs):
    if "admin_token" not in session:
      return redirect(
          url_for('admin.login_page_message', message="login in first"))
    else:
      dataBase = access_database_as_admin()
      cursor = dataBase.cursor()
      cursor.execute("select username from admins where session_token=%s",
      ...
      if cursor.fetchone() is None:
        ...
        dataBase.close()
        return redirect(
            url_for('admin.login_page_message',
                    message="no such session in db"))
      else:
        dataBase.close()
...
def get_all_requests():
  ...
  cursor = dataBase.cursor()
  cursor.execute(
      "select username,request_message,access_key from users where access_key is null or access_key!='rejected';"
  )
  ...
  for one_row in data:
    for i, column_name in enumerate(cursor.column_names):
      data_dict[column_name] = [
          one_row[i]
      ] if column_name not in data_dict else data_dict[column_name] + [
          one_row[i]
      ]
...
def update_requests():
  ...
  if request.form['mode'] == "accept":
    cursor.execute("update users set access_key=%s where username=%s",
                   [get_random_unique_access_key(), request.form['username']])
    create_user_table(
        request.form['username'])  # also add a table for this user

  elif request.form['mode'] == "reject":
    cursor.execute("update users set access_key=%s where username=%s",
                   ["rejected", request.form['username']])
    drop_user_table(request.form['username'])  # Drop table for this user

  elif request.form['mode'] == "revoke":
    cursor.execute("update users set access_key=NULL where username=%s",
                   [request.form['username']])
