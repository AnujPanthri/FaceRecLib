...
def load_settings(func):
  ...
  def wrapper_func(*args, **kwargs):
    ...
    aligner_obj.min_detection_confidence = session["demo"]['settings'][
        'a_thres']
...
def get_image_size(mode):
  if mode == 'small':
    ...
    return [
        session["demo"]['settings']['small_size'],
        session["demo"]['settings']['large_size']
    ]
  else:
    raise ("Error")
...
def set_crops():
  ...
  face_detector.image_size = get_image_size(
      session["demo"]['settings']['db_mode'])
  ...
  return jsonify({
      ...
      "image": img_base64,
      "image_name": fname,
      "crops": all_aligned_crops_base64
  })
...
def update_crops_labels():
  ...
  for img_base64 in imgs_base64:
    img = cv2.resize(base64_to_image(img_base64), [
        face_recognizer.model_config.input_size,
        face_recognizer.model_config.input_size
    ])
    all_img_features.append(
        face_recognizer.feature_extractor.predict(img[None, ...], verbose=0)[0])
    all_img_features[-1] = all_img_features[-1].astype(
        "float32").tobytes().decode("latin-1")
...
def face_recognition():
  ...
  for name in db_images:
    ...
    for decoded_features in db_images[name]:
      person_features.append(
          np.frombuffer(decoded_features.encode("latin-1"), dtype="float32"))
  ...
  face_recognizer.set_face_db_and_mode(faces=names,
                                       db_faces_features=db_faces_features,
                                       distance_mode="best",
                                       recognition_mode="repeat")

  face_detector.image_size = get_image_size(
      session["demo"]['settings']['fr_mode'])
