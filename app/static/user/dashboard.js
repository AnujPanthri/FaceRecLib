...
function dragTouchend(e, elem) {
  ...
  var droppable_element = document
    .elementFromPoint(e.changedTouches[0].clientX, e.changedTouches[0].clientY)
    .closest('.droppable');
  if (droppable_element == null) return;

  if (droppable_element.hasAttribute('data-dropto'))
    droppable_element = droppable_element.querySelector(
      droppable_element.dataset.dropto,
    );
...
function deselect(e, elem) {
  ...
  else if (elem.value != elem.dataset.name) features_updated = false;
}
...
function add_remark() {
  ...
  remark.setAttribute(
    'ondrop',
    "drop(event,this.querySelector('#remark_list>.remark>.faces'))",
  );
...
var database_input = document.querySelector(
  '#database-form>.field>.add_button>input',
);
      ...
      .then(function (response) {
        ...
        if (response['message'] == 'successful') {
          ...
          for (var i = 0; i < response['crops'].length; i++) {
            ...
            crop_img.setAttribute(
              'ontouchstart',
              'dragTouchstart(event,this);',
            );
...
function update_db_crops(elem) {
  ...
  for (var i = 0; i < face_remarks.length; i++) {
    var faces = face_remarks[i].querySelectorAll(
      '#remark_list>.remark>.faces>img',
    );
    ...
    if (faces.length > 0) {
      ...
      all_remarks[face_remarks[i].querySelector('input').value] =
        face_base64_list;
  ...
  if (num_of_remarks > 0) {
    var alldata = {};
    alldata['person_name'] = document.querySelector(
      '#database-form>.field>.person_name',
    ).value;
    ...
    fetch('set_crops/', {
      ...
      .then(function (response) {
        ...
        hide_loading_bar();
        console.log(response);
        var person_ids = document.querySelectorAll(
          '#db_people_table>tr>td:first-child',
        );
        ...
        add_person_row(
          alldata['person_name'],
          Object.keys(alldata['remarks']).join(','),
        );
      });
  } else {
...
async function face_recoginization(elem) {
  if (elem.files[0]) {
    ...
    fetch('face_recognize/', {
      ...
      .then(function (response) {
        console.log(response);
        document.querySelector('#face_recognition_image').src =
          'data:image/jpeg;base64,' + response['pred_image'];
...
function add_person_row(person_id, remarks) {
  ...
  remove_tag.innerHTML =
    '<button class="remove" onclick="delete_person(this);"><i class="fa-solid fa-close"></i></button>';
...
function close_settings(event) {
  if (
    !document.querySelector('#settings_menu').contains(event.target) &&
    !event.target.classList.contains('settings_btn')
  )
    hide_settings();
}
...
function update_settings() {
  ...
  fetch('update_settings/', {
    ...
    .then(function (res) {
      console.log(res);
      if (res['message'] == 'success') hide_settings();
    });
}
...
function reset_settings() {
  fetch('reset_settings/', {
    ...
    .then(function (res) {
      console.log(res);
      if (res['message'] == 'success') update_settings_html(res);
    });
}
