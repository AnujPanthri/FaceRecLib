...
function remove_image(e) {
  ...
  crops = document.querySelectorAll(
    `.crop_img[data-image_name='${e.dataset.image_name}']`,
  );
  // console.log(crops)
  for (var i = 0; i < crops.length; i++) crops[i].remove();
  ...
  if (elem.files[0]) {
    ...
    for (var i = 0; i < face_labels.length; i++) {
      ...
      if (faces.length > 0) {
        ...
        // name:[features_array]
        faces_features[face_labels[i].querySelector('input').value] =
          face_features;
      }
    }
    ...
    fetch('/demo/face_recognition/', {
      ...
      .then(function (response) {
        console.log(response);
        document.querySelector('#face_rec_image').src =
