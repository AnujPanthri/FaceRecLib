...
def tiler(
  ...
  for i in range(tiles):
    for j in range(tiles):
      ...
      crops.append(
          cv2.resize(img[ymin:ymax, xmin:xmax], [image_size, image_size]))
...
class square_pad:
  ...
  def rescale(self, objs_found):
    ...
    for i in range(len(objs_found)):
      objs_found[i][2] = (objs_found[i][2] - self.w_added) / (1 -
                                                              2 * self.w_added)
      objs_found[i][3] = (objs_found[i][3] - self.h_added) / (1 -
                                                              2 * self.h_added)
...
class face_detection:
  ...
  def invoke_model(self, img, p, iou_threshold, batch_size):
    ...
    for i in range(math.ceil(img.shape[0] / batch_size)):
      ...
      y_pred = self.model.predict(
          img[int(i * batch_size):int((i + 1) * batch_size)].astype('float32'),
          verbose=0)
      # print(y_pred.shape)
  ...
  def get_tiled_output(self,
    ...
    for i in range(int(tiles * tiles)):
      ...
      for j in range(all_objs_found[i].__len__()):
        ...
        all_objs_found[i][j]['xywh'] = np.array(
            all_objs_found[i][j]['xywh']) * w
        all_objs_found[i][j]['xywh'][0] += xmin
        all_objs_found[i][j]['xywh'][1] += ymin
        all_objs_found[i][j]['xywh'] = (all_objs_found[i][j]['xywh'] /
                                        (image_size * tiles)).tolist()
    ...
    if mode == "tiled":
      try:
        ...
        if "tiles" not in kwargs:
          ...
          self.tiles = kwargs['tiles'] if (
              type(kwargs['tiles']) == type(list([1])) or type(kwargs['tiles'])
              == type(np.zeros([]))) else [kwargs['tiles']]
          self.pad = kwargs['pad']
      except:

        raise ValueError(f"Not all tiled mode parameters passed.")

      self.save_tiles = kwargs["save_tiles"] if ("save_tiles"
                                                 in kwargs) else None
  ...
  def predict_once(self, img):
    ...
    if self.mode == 'tiled':
      if type(self.tiles) == type(list([1])) or type(self.tiles) == type(
          np.zeros([])):
        raise TypeError(
            "use advanced_predict function for inference on multiple tiles")
      objs_found = self.get_tiled_output(img,
      ...
      if type(self.image_size) == type(list([1])) or type(
          self.image_size) == type(np.zeros([])):
        raise TypeError(
            "use advanced_predict function for inference on multiple sizes")
      resized_img = cv2.resize(img, [self.image_size, self.image_size])
      objs_found = self.invoke_model(resized_img[None, :, :, :],
                                     self.p_thres,
                                     self.nms_thres,
                                     batch_size=1)[0]
  ...
  def predict(self, img):
    ...
    for obj_found in all_objs_found:
      obj_found[1] = idx_to_class[obj_found[1]]
